// Gfx-rs example. See: https://github.com/gfx-rs/gfx/tree/master/examples/quad
...
pub fn main_rs() -> std::os::raw::c_int {
    ...
    let pipeline = {
        let vs_module = {
            unsafe {
                device.create_shader_module(include_bytes!("../data/shaders/generated/quad.vert"))
            }
            .unwrap()
        };
        let fs_module = {
            unsafe {
                device.create_shader_module(include_bytes!("../data/shaders/generated/quad.frag"))
            }
            .unwrap()
        };
...
pub fn main_rs() -> std::os::raw::c_int {
    panic!("You need to enable the native API feature (vulkan/metal/etc)");
}
