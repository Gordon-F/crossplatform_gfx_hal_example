    ...
    B: hal::Backend,
{
    fn new(
        ...
        let pipeline = {
            let vs_module = {
                let spirv = pso::read_spirv(Cursor::new(&include_bytes!("../data/shaders/generated/quad.vert")[..]))
                    .unwrap();
                unsafe { device.create_shader_module(&spirv) }.unwrap()
            };
            let fs_module = {
                let spirv =
                    pso::read_spirv(Cursor::new(&include_bytes!("../data/shaders/generated/quad.frag")[..]))
                        .unwrap();
                unsafe { device.create_shader_module(&spirv) }.unwrap()
            };
